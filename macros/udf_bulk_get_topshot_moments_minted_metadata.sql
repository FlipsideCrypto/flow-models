{% macro udf_bulk_get_topshot_moments_minted_metadata() %}
    CREATE
    OR REPLACE EXTERNAL FUNCTION streamline.udf_bulk_get_topshot_moments_minted_metadata() returns text api_integration = aws_flow_api_dev AS {% if target.name == "prod" -%}
